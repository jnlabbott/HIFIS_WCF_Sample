    ...
    public class HomeController : Controller
    {
        ...
        private async Task UpdateAuthenticationTokenAsync(string sessionID)
        {
            if (authenticationToken == null)
            {
                ...
                authenticationToken = await (System.Threading.Tasks.Task<Authentication>)authenticationClient.ValidateUserAsync("admin", "123456", 1, "OptionalTwoFactorToken", sessionID);
            }
        }

        public async Task<IActionResult> IndexAsync()
        {
            var organizationClient = new HIFIS.Organization.OrganizationServiceClient();
            ...
            ListItem[] orgNames = await organizationClient.GetOrganizationListAsync(1, authenticationToken);
